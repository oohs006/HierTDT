...
class BertOutput(nn.Module):
    ...
    def forward(self, intermediate_output, attention_output, hidden_states=None):
        ...
        if hidden_states is not None:
            intermediate_output = self.LayerNorm(
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        for _, layer_modules in enumerate(zip(self.layer, self.layer_)):
...
class EnhancedBertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        text_extended_attention_mask = self.get_extended_attention_mask(
            text_attention_mask, text_input_shape
        )

        label_extended_attention_mask = self.get_extended_attention_mask(
