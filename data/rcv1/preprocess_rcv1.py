...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "root_dir",
        type=str,
        help="Absolute path to directory which contains rcv1.tar.xz",
    )
    ...
    pd.DataFrame(docs, columns=["id", "text", "label", "path"]).to_csv(
        str(ROOT / "rcv1_v2.csv"), index=None
    )
    logging.info("Exported data to %s", str(ROOT / "rcv1_v2.csv"))

    topic_descriptions = get_topic_desc()
    topic_descriptions = [
        {"topic_code": k, "topic_desc": v} for k, v in topic_descriptions.items()
    ]
    pd.DataFrame(topic_descriptions).to_csv(
        str(ROOT / "rcv1_v2_topics_desc.csv"), index=None
    )
    logging.info(
        "Exported topics descriptions to %s", str(ROOT / "rcv1_v2_topics_desc.csv")
    )
